...
def parse_args():
    ...
    parser.add_argument('--init_id', type=int, default=2, help='Initial number of camera ID')
    parser.add_argument('--all_cam_num', type=int, default=2, help='Number of cameras in total')
    parser.add_argument('--cam_id_gap', type=int, default=2, help='Gap between two camera IDs')
    parser.add_argument('--visualization', type=bool, default=False, help='Display visualization or not')
...
class MultiCameraSystem:
    def __init__(self, args, cam_ids, width=640, height=480, fps=30.0):
        ...
        if not args.skip_records:
            ...
            plt.ion()
            self.figure, self.axes, self.sub_imgs = self.visual_cams() # get visual results
            ...
            # while time.time() - st < 180 and self.vis:
            while time.time() - st < 180:
                if self.vis:
                    ...
                    continue
            self.stop_all_cams()
        self.sync_streams(vis=self.vis)
        if self.vis:
    ...
    def sync_streams(self,vis=True):
...
if __name__ == '__main__':
    args = parse_args()
    cam_id_list = []
    all_cam_num = args.all_cam_num
    # init_id = 2
    init_id = args.init_id
    for cam_idx in range(all_cam_num):
        cam_id_list.append(f'/dev/video{init_id + cam_idx * args.cam_id_gap}')
